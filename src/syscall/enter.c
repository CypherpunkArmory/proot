...
#define PR_GET_AUXV 0x41555856
	...
	case PR_ioctl: {
		...
		/* Keep this fork's TCSETSF2 -> TCSETSF mapping rather than upstream's
		 * TCSETSF2 -> TCSETS: the other three rewrites just above preserve the
		 * variant (TCGETS2->TCGETS, TCSETS2->TCSETS, TCSETSW2->TCSETSW), and
		 * TCSETSF is the flush-on-set form, so mapping it to plain TCSETS would
		 * silently drop the input-flush this ioctl exists to request.  */
		if (cmd == TCSETSF2)
